...
function impactForLicenses(n) {
  ...
  return { level: null, color: "#64748b" }
}
...
export default function ExitSimulator() {
  ...
  const coveredAfter = useMemo(() => {
    const set = new Set()
    remaining.forEach((p) => getActiveStates(p).forEach((s) => set.add(s)))
    return set
  }, [remaining])
  ...
  const thinned = useMemo(
    () =>
      patientStates.filter((s) => {
        const before = activeProviders.filter((p) => getActiveStates(p).includes(s)).length
        const after = remaining.filter((p) => getActiveStates(p).includes(s)).length
  ...
  const licensesLost = useMemo(() => exitingList.reduce((n, p) => n + getActiveStates(p).length, 0), [exitingList])
  const { level: impactLevel, color: impactColor } = impactForLicenses(licensesLost)

  const riskRanking = useMemo(
    () => [...activeProviders].sort((a, b) => getActiveStates(b).length - getActiveStates(a).length),
    [activeProviders]
  )
  ...
  return (
    <div style={{ padding: 24, background: theme.bg0, minHeight: "100vh", color: theme.text, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <p style={{ margin: "0 0 12px", fontSize: 14, color: theme.muted }}>Click &quot;Simulate&quot; to add a provider to the exit scenario.</p>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${theme.border1}` }}>
                <th style={{ textAlign: "left", padding: 10, color: theme.muted }}>Provider</th>
                <th style={{ textAlign: "right", padding: 10, color: theme.muted }}>Licenses</th>
                <th style={{ textAlign: "left", padding: 10, color: theme.muted }}>Impact</th>
                <th style={{ textAlign: "left", padding: 10 }} />
              </tr>
            </thead>
            <tbody>
              {riskRanking.map((p) => {
                const { level, color } = impactForLicenses(getActiveStates(p).length)
                return (
                  <tr key={p.id} style={{ borderBottom: `1px solid ${theme.border2}` }}>
                    <td style={{ padding: 10 }}>
                      <CredBadge type={p.type} /> {p.name}
                    </td>
                    <td style={{ padding: 10, textAlign: "right" }}>{getActiveStates(p).length}</td>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 24, marginBottom: 24 }}>
        <div>
          <label style={{ display: "block", fontSize: 12, color: theme.muted, marginBottom: 6 }}>Exiting providers (multi-select)</label>
            ...
            {activeProviders.map((p) => (
              <button
                ...
                style={{
                  padding: "8px 12px",
                  borderRadius: 8,
                  border: `2px solid ${providerIds.includes(p.id) ? "#dc2626" : theme.border1}`,
                  background: providerIds.includes(p.id) ? "#451a1a" : theme.bg2,
                  color: theme.text,
                  cursor: "pointer",
                  display: "inline-flex",
                  alignItems: "center",
      ...
      {exitingList.length === 0 && (
        <p style={{ color: theme.muted, fontSize: 14 }}>Select one or more providers above (or use Simulate in the risk table) to see exit impact.</p>
      )}

      {exitingList.length > 0 && (
        <>
          <div style={{ marginBottom: 16, padding: 12, background: theme.bg2, borderRadius: 8, borderLeft: `4px solid ${theme.accent}` }}>
            <strong>Simulating exit:</strong> {exitingList.map((p) => `${p.name} (${p.type})`).join(", ")}
          </div>
          <div style={{ marginBottom: 24, padding: 16, background: theme.bg2, borderRadius: 8, border: `2px solid ${impactColor}` }}>
            <div style={{ fontSize: 12, color: theme.muted, marginBottom: 4 }}>Total licenses lost (gap impact)</div>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
            <Card>
              <div style={{ fontSize: 12, color: theme.muted }}>New critical gaps</div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>{newCritical.length}</div>
            </Card>
            <Card>
              <div style={{ fontSize: 12, color: theme.muted }}>States thinned</div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>{thinned.length}</div>
            </Card>
            <Card>
              <div style={{ fontSize: 12, color: theme.muted }}>HV states newly at zero</div>
